organization = {
    ...
    initIndexPage: function () {
		...
		fpcs.jqGrid.initGrid({
			...
			localStorageId: "Organization",
			url: "/Organization/_Index",
			rowNum: 100000,
		...
		fpcs.jqGrid.initFilterToolbar("Organization");
		if (fpcs.getIsAdmin()) {
		    fpcs.jqGrid.initNavButtons("/Organization/DeleteAll", organization.showCreateDialog, "Добавить новую запись");
		} else {
		...
		organization.initCreateDialogSend();
		organization.initEditDialog();
		organization.initDeleteOneEntity();
		// fizPerson.sendEmail();
	},
	...
	showCreateDialog: function () {
		fpcs.getPartial('/Organization/_Create/', function (data, textStatus) {
	...
	initCreateDialogSend: function () {
		$(document).off("click", ".createOrganizationSend");
		$(document).on("click", ".createOrganizationSend", function (e) {
			e.preventDefault();
			fpcs.sendForm("createOrganizationForm", function (data, textStatus) {
				if (typeof data === "object" && data.ErrorCode === 200) {
					organization.reloadGrid();
					fpcs.closeDialog();
				}
				else {
					fpcs.showDialog("Добавить новую запись", data);
	...
	showEditDialog: function (id) {
		fpcs.getPartial('/Organization/_Edit/' + id, function (data, textStatus) {
	...
	initEditDialog: function () {
		...
		$(document).on("click", ".gridRowEdit", function (e) {
			var id = $(this).attr("rowid");
			organization.showEditDialog(id);
		});

		$(document).off("click", ".editOrganizationSend");
		$(document).on("click", ".editOrganizationSend", function (e) {
			e.preventDefault();
			fpcs.sendForm("editOrganizationForm", function (data, textStatus) {
				if (typeof data === "object" && data.ErrorCode === 200) {
					organization.reloadGrid();
					fpcs.closeDialog();
				}
