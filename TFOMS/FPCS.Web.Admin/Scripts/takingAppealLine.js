takingAppealLine = {
    ...
    initIndexPage: function () {
		...
		fpcs.jqGrid.initGrid({
			...
			localStorageId: "TakingAppealLine",
			url: "/TakingAppealLine/_Index",
			rowNum: 100000,
		...
		fpcs.jqGrid.initFilterToolbar("TakingAppealLine");
		if (fpcs.getIsAdmin()) {
		    fpcs.jqGrid.initNavButtons("/TakingAppealLine/DeleteAll", takingAppealLine.showCreateDialog, "Добавить новую запись");
		} else {
		...
		takingAppealLine.initCreateDialogSend();
		takingAppealLine.initEditDialog();
		takingAppealLine.initDeleteOneEntity();
		// fizPerson.sendEmail();
	},
	...
	showCreateDialog: function () {
		fpcs.getPartial('/TakingAppealLine/_Create/', function (data, textStatus) {
	...
	initCreateDialogSend: function () {
		$(document).off("click", ".createTakingAppealLineSend");
		$(document).on("click", ".createTakingAppealLineSend", function (e) {
			e.preventDefault();
			fpcs.sendForm("createTakingAppealLineForm", function (data, textStatus) {
				if (typeof data === "object" && data.ErrorCode === 200) {
					takingAppealLine.reloadGrid();
					fpcs.closeDialog();
				}
				else {
					fpcs.showDialog("Добавить новую запись", data);
	...
	showEditDialog: function (id) {
		fpcs.getPartial('/TakingAppealLine/_Edit/' + id, function (data, textStatus) {
	...
	initEditDialog: function () {
		...
		$(document).on("click", ".gridRowEdit", function (e) {
			var id = $(this).attr("rowid");
			takingAppealLine.showEditDialog(id);
		});

		$(document).off("click", ".editTakingAppealLineSend");
		$(document).on("click", ".editTakingAppealLineSend", function (e) {
			e.preventDefault();
			fpcs.sendForm("editTakingAppealLineForm", function (data, textStatus) {
				if (typeof data === "object" && data.ErrorCode === 200) {
					takingAppealLine.reloadGrid();
					fpcs.closeDialog();
				}
	...
	initDeleteOneEntity: function () {
		...
		$(document).on("click", ".gridRowDelete", function (e) {
			var id = $(this).attr("rowid");
			var url = "/TakingAppealLine/Delete/" + id;
