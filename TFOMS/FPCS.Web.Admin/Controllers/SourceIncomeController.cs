...
{
    public class SourceIncomeController : BaseController
    {
        ...
        public JsonResult _Index(GridOptions options, DictionaryListOptions dictionaryListOptions)
        {
            ...
            {
                var repo = uow.GetRepo<ISourseIncomeRepo>();

                var sourseIncome = repo.GetAll()
                    .Select(x => new
                    {
                        Id = x.SourceIncomeId,
                        x.Code,
                ...
                var resultTemp = engine.ApplySort(engine.ApplyFilter(sourseIncome.AsQueryable())).Select(x => new DictionaryIndexModel
                {
                    Id = x.Id,
                    Code = x.Code,
                    Name = x.Name
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(dbEntity.SourceIncomeId.ToString());
                    }
                }
                catch (Exception ex)
                {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(new { PersonId = dbEntity.SourceIncomeId });
                    }
                }
                catch (Exception ex)
                {
