...
{
    public class PassedEventController : BaseController
    {
        ...
        public JsonResult _Index(GridOptions options, DictionaryListOptions dictionaryListOptions)
        {
            ...
            {
                var repo = uow.GetRepo<IPassedEventRepo>();

                var passedEvent = repo.GetAll()
                    .Select(x => new
                    {
                        Id = x.PassedEventId,
                        x.Code,
                ...
                var resultTemp = engine.ApplySort(engine.ApplyFilter(passedEvent.AsQueryable())).Select(x => new DictionaryIndexModel
                {
                    Id = x.Id,
                    Code = x.Code,
                    Name = x.Name
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(dbEntity.PassedEventId.ToString());
                    }
                }
                catch (Exception ex)
                {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(new { PersonId = dbEntity.PassedEventId });
                    }
                }
                catch (Exception ex)
                {
