...
{
    public class TakingAppealLineController : BaseController
    {
        ...
        public JsonResult _Index(GridOptions options, DictionaryListOptions dictionaryListOptions)
        {
            ...
            {
                var repo = uow.GetRepo<ITakingAppealLineRepo>();

                var takingAppealLine = repo.GetAll()
                    .Select(x => new
                    {
                        Id = x.TakingAppealLineId,
                        x.Code,
                        x.Name
                    })
                    .ToList();
                ...
                var resultTemp = engine.ApplySort(engine.ApplyFilter(takingAppealLine.AsQueryable())).Select(x => new DictionaryIndexModel
                {
                    Id = x.Id,
                    Code = x.Code,
                    Name = x.Name
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(dbEntity.TakingAppealLineId.ToString());
                    }
                }
                catch (Exception ex)
                {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(new { PersonId = dbEntity.TakingAppealLineId });
                    }
                }
                catch (Exception ex)
                {
