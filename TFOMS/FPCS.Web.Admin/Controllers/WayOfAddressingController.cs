...
{
    public class WayOfAddressingController : BaseController
    {
        ...
        public JsonResult _Index(GridOptions options, DictionaryListOptions dictionaryListOptions)
        {
            ...
            {
                var repo = uow.GetRepo<IWayOfAddressingRepo>();

                var wayOfAddressing = repo.GetAll()
                    .Select(x => new
                    {
                        Id = x.WayOfAddressingId,
                        x.Code,
                        x.Name
                    })
                    .ToList();
                ...
                var resultTemp = engine.ApplySort(engine.ApplyFilter(wayOfAddressing.AsQueryable())).Select(x => new DictionaryIndexModel
                {
                    Id = x.Id,
                    Code = x.Code,
                    Name = x.Name
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(dbEntity.WayOfAddressingId.ToString());
                    }
                }
                catch (Exception ex)
                {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(new { PersonId = dbEntity.WayOfAddressingId });
                    }
                }
                catch (Exception ex)
                {
