...
{
    public class AppealResultController : BaseController
    {
        ...
        public JsonResult _Index(GridOptions options, DictionaryListOptions dictionaryListOptions)
        {
            ...
            {
                var repo = uow.GetRepo<IAppealResultRepo>();

                var appealResult = repo.GetAll()
                    .Select(x => new
                    {
                        Id = x.AppealResultId,
                        x.Code,
                ...
                var resultTemp = engine.ApplySort(engine.ApplyFilter(appealResult.AsQueryable())).Select(x => new DictionaryIndexModel
                {
                    Id = x.Id,
                    Code = x.Code,
                    Name = x.Name
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(dbEntity.AppealResultId.ToString());
                    }
                }
                catch (Exception ex)
                {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(new { PersonId = dbEntity.AppealResultId });
                    }
                }
                catch (Exception ex)
                {
