...
{
    public class ReviewAppealLineController : BaseController
    {
        ...
        public JsonResult _Index(GridOptions options, DictionaryListOptions dictionaryListOptions)
        {
            ...
            {
                var repo = uow.GetRepo<IReviewAppealLineRepo>();

                var reviewAppealLine = repo.GetAll()
                    .Select(x => new
                    {
                        Id = x.ReviewAppealLineId,
                        x.Code,
                        x.Name
                    })
                    .ToList();
                ...
                var resultTemp = engine.ApplySort(engine.ApplyFilter(reviewAppealLine.AsQueryable())).Select(x => new DictionaryIndexModel
                {
                    Id = x.Id,
                    Code = x.Code,
                    Name = x.Name
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(dbEntity.ReviewAppealLineId.ToString());
                    }
                }
                catch (Exception ex)
                {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(new { PersonId = dbEntity.ReviewAppealLineId });
                    }
                }
                catch (Exception ex)
                {
