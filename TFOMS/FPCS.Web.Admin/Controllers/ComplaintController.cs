...
namespace FPCS.Web.Admin.Controllers
{
    public class ComplaintController : BaseController
    {
        ...
        public JsonResult _Index(GridOptions options, DictionaryListOptions dictionaryListOptions)
        {
            ...
            {
                var repo = uow.GetRepo<IComplaintRepo>();

                var complaint = repo.GetAll()
                    .Select(x => new
                    {
                        Id = x.ComplaintId,
                        x.Code,
                ...
                var resultTemp = engine.ApplySort(engine.ApplyFilter(complaint.AsQueryable())).Select(x => new DictionaryIndexModel
                {
                    Id = x.Id,
                    Code = x.Code,
                    Name = x.Name
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(dbEntity.ComplaintId.ToString());
                    }
                }
                catch (Exception ex)
                {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var uow = UnityManager.Resolve<IUnitOfWork>())
                    {
                        ...
                        return JsonRes(new { PersonId = dbEntity.ComplaintId });
                    }
                }
                catch (Exception ex)
                {
